  ...
  return (
    <div>
      <h2 id="masterdata-heading" data-cy="MasterdataHeading">
        <Translate contentKey="warehouseMgmApp.masterdata.home.title">Masterdata</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="warehouseMgmApp.masterdata.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/masterdata/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="warehouseMgmApp.masterdata.home.createLabel">Create new Masterdata</Translate>
          </Link>
        </div>
      </h2>
        ...
        {masterdataList && masterdataList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="warehouseMgmApp.masterdata.id">ID</Translate>{' '}
            ...
            <tbody>
              {masterdataList.map((masterdata, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{masterdata.createdAt ? <TextFormat type="date" value={masterdata.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{masterdata.createdBy}</td>
                  <td>{masterdata.updatedAt ? <TextFormat type="date" value={masterdata.updatedAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{masterdata.updatedBy}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/masterdata/${masterdata.id}`} color="info" size="sm" data-cy="entityDetailsButton">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="warehouseMgmApp.masterdata.home.notFound">No Masterdata found</Translate>
            </div>
          )
        )}
      </div>
