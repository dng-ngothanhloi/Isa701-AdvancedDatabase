  ...
  return (
    <div>
      <h2 id="khach-hang-heading" data-cy="KhachHangHeading">
        <Translate contentKey="warehouseMgmApp.khachHang.home.title">Khach Hangs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="warehouseMgmApp.khachHang.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/khach-hang/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="warehouseMgmApp.khachHang.home.createLabel">Create new Khach Hang</Translate>
          </Link>
        </div>
      </h2>
        ...
        {khachHangList && khachHangList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('maKH')}>
                  <Translate contentKey="warehouseMgmApp.khachHang.maKH">Ma KH</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('maKH')} />
                </th>
                <th className="hand" onClick={sort('tenKH')}>
                  <Translate contentKey="warehouseMgmApp.khachHang.tenKH">Ten KH</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('tenKH')} />
                </th>
                <th className="hand" onClick={sort('goiTinh')}>
                  <Translate contentKey="warehouseMgmApp.khachHang.goiTinh">Goi Tinh</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('goiTinh')} />
                </th>
                <th className="hand" onClick={sort('dateOfBirth')}>
                  <Translate contentKey="warehouseMgmApp.khachHang.dateOfBirth">Date Of Birth</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('dateOfBirth')} />
                </th>
                <th className="hand" onClick={sort('diaChi')}>
                  <Translate contentKey="warehouseMgmApp.khachHang.diaChi">Dia Chi</Translate>{' '}
            ...
            <tbody>
              {khachHangList.map((khachHang, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`warehouseMgmApp.EmpSex.${khachHang.goiTinh}`} />
                  </td>
                  <td>
                    {khachHang.dateOfBirth ? <TextFormat type="date" value={khachHang.dateOfBirth} format={APP_LOCAL_DATE_FORMAT} /> : null}
                  </td>
                  <td>{khachHang.diaChi}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/khach-hang/${khachHang.id}`} color="info" size="sm" data-cy="entityDetailsButton">
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="warehouseMgmApp.khachHang.home.notFound">No Khach Hangs found</Translate>
            </div>
          )
        )}
      </div>
