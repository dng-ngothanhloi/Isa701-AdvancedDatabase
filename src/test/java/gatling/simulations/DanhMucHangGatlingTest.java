...
public class DanhMucHangGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all danhMucHangs").get("/api/danh-muc-hangs").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new danhMucHang")
                        .post("/api/danh-muc-hangs")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_danhMucHang_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created danhMucHang").get("${new_danhMucHang_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created danhMucHang").delete("${new_danhMucHang_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DanhMucHang entity").exec(scn);
