...
public class PhieuNhapXuatGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
            ...
            .repeat(2)
            .on(
                    exec(http("Get all phieuNhapXuats").get("/api/phieu-nhap-xuats").headers(headersHttpAuthenticated).check(status().is(200)))
                            .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                            .exec(
                                    http("Create new phieuNhapXuat")
                                            .post("/api/phieu-nhap-xuats")
                                            .headers(headersHttpAuthenticated)
                                            .body(
                                                    StringBody(
                                                            "{" +
                                                                    "\"maPhieu\": \"SAMPLE_TEXT\"" +
                                                                    ", \"ngayLapPhieu\": \"2020-01-01T00:00:00.000Z\"" +
                                                                    ", \"loaiPhieu\": \"Nhap\"" +
                                            ...
                                            .asJson()
                                            .check(status().is(201))
                                            .check(headerRegex("Location", "(.*)").saveAs("new_phieuNhapXuat_url"))
                            )
                            .exitHereIfFailed()
                            .pause(10)
                            .repeat(5)
                            .on(exec(http("Get created phieuNhapXuat").get("${new_phieuNhapXuat_url}").headers(headersHttpAuthenticated)).pause(10))
                            .exec(http("Delete created phieuNhapXuat").delete("${new_phieuNhapXuat_url}").headers(headersHttpAuthenticated))
                            .pause(10)
            );

    ScenarioBuilder users = scenario("Test the PhieuNhapXuat entity").exec(scn);
