...
public class MasterdataGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
            ...
            .repeat(2)
            .on(
                    exec(http("Get all masterdata").get("/api/masterdata").headers(headersHttpAuthenticated).check(status().is(200)))
                            .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                            .exec(
                                    http("Create new masterdata")
                                            .post("/api/masterdata")
                                            ...
                                            .asJson()
                                            .check(status().is(201))
                                            .check(headerRegex("Location", "(.*)").saveAs("new_masterdata_url"))
                            )
                            .exitHereIfFailed()
                            .pause(10)
                            .repeat(5)
                            .on(exec(http("Get created masterdata").get("${new_masterdata_url}").headers(headersHttpAuthenticated)).pause(10))
                            .exec(http("Delete created masterdata").delete("${new_masterdata_url}").headers(headersHttpAuthenticated))
                            .pause(10)
            );

    ScenarioBuilder users = scenario("Test the Masterdata entity").exec(scn);
