...
public class ChiTietNhapXuatGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
            ...
            .repeat(2)
            .on(
                    exec(http("Get all chiTietNhapXuats").get("/api/chi-tiet-nhap-xuats").headers(headersHttpAuthenticated).check(status().is(200)))
                            .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                            .exec(
                                    http("Create new chiTietNhapXuat")
                                            .post("/api/chi-tiet-nhap-xuats")
                                            ...
                                            .asJson()
                                            .check(status().is(201))
                                            .check(headerRegex("Location", "(.*)").saveAs("new_chiTietNhapXuat_url"))
                            )
                            .exitHereIfFailed()
                            .pause(10)
                            .repeat(5)
                            .on(exec(http("Get created chiTietNhapXuat").get("${new_chiTietNhapXuat_url}").headers(headersHttpAuthenticated)).pause(10))
                            .exec(http("Delete created chiTietNhapXuat").delete("${new_chiTietNhapXuat_url}").headers(headersHttpAuthenticated))
                            .pause(10)
            );

    ScenarioBuilder users = scenario("Test the ChiTietNhapXuat entity").exec(scn);
